...
__global__ void
createResizedImage(unsigned char *imageScaledData, int scaled_width, float scale_factor, cudaTextureObject_t texObj) {
...
//    printf("\n%s", cudaGetErrorString(returnValue));
    ...
    returnValue = (cudaError_t)(returnValue |
                                cudaMemcpyToArray(cu_array, 0, 0, data, height * width * sizeof(unsigned char),
...
//    printf("\n Launching grid with blocks [%d][%d] ", dimGrid.x, dimGrid.y);
    ...
    if (returnValue != cudaSuccess)
        fprintf(stderr, "Got error while running CUDA API kernel\n");
    ...
    std::cout << boost::format("t=%1%:n=%2%:x=%3%:y=%4%\n") % gpuTimer.elapsed() % _args->vector_size % dimGrid.x % dimGrid.y;
